...
if [ "$(uname -m)" = "x86_64" ]; then
    LOG_DETAIL=$HOME_DIR"/x86_hdd_fio_detail.log"
    LOG_SIMPLE=$HOME_DIR"/x86_hdd_fio_simple.log"
    DEV_NAME="/dev/sdb"
    echo "x86_64 set $DEV_NAME"
    
elif [ "$(uname -m)" = "aarch64" ]; then
    LOG_DETAIL=$HOME_DIR"/arm_hdd_fio_detail.log"
    LOG_SIMPLE=$HOME_DIR"/arm_hdd_fio_simple.log"
    DEV_NAME="/dev/sdb"
    ...
    for ro in ${rw[@]}; do
		for n in ${numjob[@]}; do
			for d in ${iodepth[@]}; do
                ...
                printf "%s\t %s\t %8s\t %s\t %s\t\n" $iops $b $lat $cpu_user $cpu_sys | tee -a $LOG_SIMPLE
                echo "" >> $LOG_DETAIL
                (( count++ ))  
			done
		done
